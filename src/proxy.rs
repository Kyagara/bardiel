...
pub async fn handle_connection(
    ...
    if handshake.next_state == NextState::Status {
        ...
        ServerStatusResponse::handle_server_status_request(&mut client_conn, server_status).await?;
        return Ok(());
    }
    ...
    match TcpStream::connect(proxy_config.server_address).await {
        Ok(mut server_conn) => {
            ...
            if handshake.next_state == NextState::Login {
                ...
                username = login::get_client_username(&mut client_conn, &mut server_conn).await?;
